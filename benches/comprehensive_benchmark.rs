...
use sentencex::{segment, segment_borrowed, get_sentence_boundaries};
...
fn bench_segment_borrowed_by_size(c: &mut Criterion) {
    let mut group = c.benchmark_group("segment_borrowed_by_size");
    
    for size in [100, 1_000, 10_000, 50_000].iter() {
        let text = generate_text(*size);
        group.throughput(Throughput::Bytes(*size as u64));
        group.bench_with_input(BenchmarkId::from_parameter(size), &text, |b, text| {
            b.iter(|| segment_borrowed(black_box("en"), black_box(text)))
        });
    }
...
fn bench_multi_language(c: &mut Criterion) {
    ...
    let languages = vec![
        ("en", "This is an English sentence. Another one follows. And yet another!"),
        ("fr", "Ceci est une phrase en français. Une autre suit. Et encore une autre!"),
        ("de", "Dies ist ein deutscher Satz. Ein weiterer folgt. Und noch einer!"),
        ("es", "Esta es una oración en español. Otra sigue. ¡Y otra más!"),
        ("ja", "これは日本語の文です。もう一つあります。そしてもう一つ。"),
        ("ar", "هذه جملة باللغة العربية. تتبعها جملة أخرى. وجملة أخرى!"),
    ];
...
criterion_group!(
    ...
    bench_segment_borrowed_by_size,
